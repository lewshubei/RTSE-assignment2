...
def send_controls_task():
    ...
    if trailing_detected:
        dodge_options = [l for l in [-2, -1, 0, 1, 2]
                         if l not in completely_blocked_lanes and l != current_lane]
        if dodge_options:
            chosen_target_lane = min(dodge_options, key=lambda l: abs(l - current_lane))
            decision_reason = "TRAILING_CAR_DODGE"
            print(f"[CONTROL] Trailing car! Dodging to lane {chosen_target_lane}")

    # Police car: must collect the nearest red token to satisfy the officer
    if police_detected:
