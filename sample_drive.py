...
POLICE_MIN_RED_PIXELS = 150
POLICE_MIN_BLUE_PIXELS = 150
...
def detect_police_car(frame):
    ...
    for rx, ry, rw, rh, rarea in red_regions:
        for bx, by, bw, bh, barea in blue_regions:
            ...
            # Horizontal alignment check: The centers must be horizontally adjacent, not vertically stacked
            if abs(center_red_y - center_blue_y) * 1.5 > abs(center_red_x - center_blue_x):
                continue
                
            # Spatial separation: The centers must be distinctly separated horizontally
            if abs(center_red_x - center_blue_x) < 15:
                continue
